...
def load_data(save_path=SAVE_PATH):
    if not os.path.exists(f"data/10_ep/train_loader.pth") or not os.path.exists(f"data/10_ep/val_loader.pth") or not os.path.exists(f"data/10_ep/input_dim.json"):
        return data_processing.data_processing_h5(save_path=save_path)

    train_loader = torch.load(f"data/10_ep/train_loader.pth", weights_only=False)
    val_loader = torch.load(f"data/10_ep/val_loader.pth", weights_only=False)
    with open(f"data/10_ep/input_dim.json", "r") as f:
        input_dim = json.load(f)

    try:
        os.makedirs(save_path, exist_ok=True)
        shutil.copy(f"data/10_ep/x_mean.npy", f"{save_path}/x_mean.npy")
        shutil.copy(f"data/10_ep/x_std.npy", f"{save_path}/x_std.npy")
        shutil.copy(f"data/10_ep/y_mean.npy", f"{save_path}/y_mean.npy")
        shutil.copy(f"data/10_ep/y_std.npy", f"{save_path}/y_std.npy")
