...
def eval_genome(genome, config, n_episodes):
    env = gym.make('Meta-World/MT1', env_name='pick-place-v3', max_episode_steps=STEP_LIMIT)
    ...
    for episode_id in range(n_episodes):
        ...
        for step in range(STEP_LIMIT):
            observations.append(obs[:36])
            action = np.clip(net.activate(obs[:36]), -1.0, 1.0)
    ...
    env.close()
    return np.mean(final_rewards), genome_data #np.mean(total_rewards)
