...
def load_data(save_path=SAVE_PATH):
    if not os.path.exists(f"data/2_ep_20_feat/train_loader.pth") or not os.path.exists(f"data/2_ep_20_feat/val_loader.pth") or not os.path.exists(f"data/2_ep_20_feat/input_dim.json"):
        return data_processing.data_processing_h5(save_path=save_path)

    train_loader = torch.load(f"data/2_ep_20_feat/train_loader.pth", weights_only=False)
    val_loader = torch.load(f"data/2_ep_20_feat/val_loader.pth", weights_only=False)
    with open(f"data/2_ep_20_feat/input_dim.json", "r") as f:
        input_dim = json.load(f)

    try:
        os.makedirs(save_path, exist_ok=True)
        shutil.copy(f"data/2_ep_20_feat/x_mean.npy", f"{save_path}/x_mean.npy")
        shutil.copy(f"data/2_ep_20_feat/x_std.npy", f"{save_path}/x_std.npy")
        shutil.copy(f"data/2_ep_20_feat/y_mean.npy", f"{save_path}/y_mean.npy")
        shutil.copy(f"data/2_ep_20_feat/y_std.npy", f"{save_path}/y_std.npy")
...
if __name__ == "__main__":
    ...
    save_path = f"models/{model_folder}_transformer"
