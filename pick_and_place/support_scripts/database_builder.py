...
def eval_genome(genome, config, n_episodes):
    env = gym.make('FetchPickAndPlace-v4', max_episode_steps=STEP_LIMIT)
    ...
    env.close()
    return reward, genome_data #np.mean(total_rewards)
    ...
    with h5py.File(H5_PATH, "a") as h5file:
        for genome_data in population_data:
            ...
            for episode_id, episode_data in enumerate(genome_data["episodes_data"]):
                ...
                episode_group.create_dataset("observations", data=np.array(episode_data["observations"]))
                episode_group.create_dataset("actions", data=np.array(episode_data["actions"]))
