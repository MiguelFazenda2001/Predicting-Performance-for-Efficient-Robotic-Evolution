...
MODEL_PATH = "best_training_transformer_model.pth"
...
# Alternatively, load from preprocessed .pth and .json files
data_loader = torch.load(f"../../data/train_loader.pth", weights_only=False)
with open(INPUT_DIM, "r") as f:
    input_dim = json.load(f)


# ---------------- LOAD MODEL ----------------
model = EpisodeTransformer(input_dim=input_dim).to(device)
